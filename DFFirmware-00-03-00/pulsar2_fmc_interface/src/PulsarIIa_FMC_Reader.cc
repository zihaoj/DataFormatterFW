...
// for FMC interface
// --------------------------------
// Note to modify the version between Fermilab test mezzanine version and FTK_IM
// (1) the following two will be controlled by the argument
// (1-1) Position of the clock forwarding  LA01 (FNAL TM) <--> LA17 (FTK_IM)
// (1-2) all signals are assigned to LA (FNAL TM) or some of them are assigned to HA (FTK_IM)
// (2) in the pulsariia_integration_test_2014feb/top_df_main.vhd modify the following two lines:
// fmc_config_fb1_soft_reset1_out <= fmc_config_user_interface_i(0) xor swap_mask_fmc1_soft_reset1;--tmp
// fmc_config_fb2_soft_reset1_out <= fmc_config_user_interface_i(0) xor swap_mask_fmc2_soft_reset1;--tmp
// ========================================================
...
void set(bool DO_SPECIAL_CONFIGURATION_FOR_IM_TEST)
{
  ...
  rx_fmc_to_fpga1_bit_mapping.insert(std::pair<int, FMC_CHANNEL_ID>( 8, LA19)); // channel in FPGA - channel in FMC # HA19
  rx_fmc_to_fpga1_bit_mapping.insert(std::pair<int, FMC_CHANNEL_ID>( 9, LA21)); // channel in FPGA - channel in FMC # HA16
  rx_fmc_to_fpga1_bit_mapping.insert(std::pair<int, FMC_CHANNEL_ID>(10, LA27)); // channel in FPGA - channel in FMC # HA12
  rx_fmc_to_fpga1_bit_mapping.insert(std::pair<int, FMC_CHANNEL_ID>(11, LA30)); // channel in FPGA - channel in FMC # HA23
...
int main(int argc, char* argv[])
{
  ...
  if (argc!=2) {
    std::cout << "usage ./iia.exe <1:FTK_IM or 0:FNAL_TM> " << std::endl;
    exit(EXIT_FAILURE);
  ...
  if (DO_SPECIAL_CONFIGURATION_FOR_IM_TEST) {
    std::cout << "###########################################" << std::endl;
    std::cout << "## SPECIAL CONFIGURATION with HA is used ##" << std::endl;
    std::cout << "###########################################" << std::endl;
  }
  ...
  while (not (input.eof())) {
    ...
    const std::string fmcHL_str(fmcHL);
    ...
    } else { // special configuration !! see the table of RX mapping
      // (1) skip LA19 but use HA19 instead as if it is LA19
      // (2) skip LA21 but use HA16 instead as if it is LA21
      // (3) skip LA27 but use HA12 instead as if it is LA27
      // (4) skip LA30 but use HA23 instead as if it is LA30
      if        (fmcHL_str=="L") { //
	if (chan==LA19) continue;
	if (chan==LA21) continue;
	if (chan==LA27) continue;
	if (chan==LA30) continue;
      } else if (fmcHL_str=="H") { // don't care for fmcHL_str"CLK"
	if      (chan==19) { chan=LA19;}
	else if (chan==16) { chan=LA21;}
	else if (chan==12) { chan=LA27;}
	else if (chan==23) { chan=LA30;}
	else continue;
      }
    }
    //std::cout << chan << " " << line << std::endl;
...
void dump_ucf()
{
  ...
  for (int iChan=0; iChan<rx_fmc_to_fpga0_bit_mapping.size(); iChan++) { // FPGA0
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_RX_LVDS_P[" << iChan << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_RX_LVDS_N[" << iChan << "] ]" << std::endl;
  ...
  for (int iChan=0; iChan<rx_fmc_to_fpga1_bit_mapping.size(); iChan++) { // FPGA1
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_RX_LVDS_P[" << channel_id_total << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_RX_LVDS_N[" << channel_id_total << "] ]" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_CLK0_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_CLK0_N]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_CLK1_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_CLK1_N]" << std::endl;
  ...
  for (int iChan=0; iChan<rx_fmc_to_fpga0_bit_mapping.size(); iChan++) { // FPGA0
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_RX_LVDS_P[" << iChan << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_RX_LVDS_N[" << iChan << "] ]" << std::endl;
  ...
  for (int iChan=0; iChan<rx_fmc_to_fpga1_bit_mapping.size(); iChan++) { // FPGA1
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_RX_LVDS_P[" << channel_id_total << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_RX_LVDS_N[" << channel_id_total << "] ]" << std::endl;   
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_CLK0_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_CLK0_N]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_CLK1_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_CLK1_N]" << std::endl;
  ...
  for (int iChan=0; iChan<tx_fmc_to_fpga0_bit_mapping.size(); iChan++) { // FPGA0
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_TX_LVDS_P[" << iChan << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_TX_LVDS_N[" << iChan << "] ]" << std::endl;
  ...
  for (int iChan=0; iChan<tx_fmc_to_fpga1_bit_mapping.size(); iChan++) { // FPGA1
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_TX_LVDS_P[" << channel_id_total << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_TX_LVDS_N[" << channel_id_total << "] ]" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_TX_FWD_CLK_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_TX_FWD_CLK_N]" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_SOFT_RESET0_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_SOFT_RESET0_N]" << std::endl;
  //std::cout<<"NET FB1_SOFT_RESET0_P LOC="<<(pins_pos_la_fmcs[0])[soft_reset_fmc_to_fpga_bit_mapping[0]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  //std::cout<<"NET FB1_SOFT_RESET0_N LOC="<<(pins_neg_la_fmcs[0])[soft_reset_fmc_to_fpga_bit_mapping[0]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_SOFT_RESET1_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB1_SOFT_RESET1_N]" << std::endl;
  //std::cout<<"NET FB1_SOFT_RESET1_P LOC="<<(pins_pos_la_fmcs[0])[soft_reset_fmc_to_fpga_bit_mapping[1]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  //std::cout<<"NET FB1_SOFT_RESET1_N LOC="<<(pins_neg_la_fmcs[0])[soft_reset_fmc_to_fpga_bit_mapping[1]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  ...
  for (int iChan=0; iChan<tx_fmc_to_fpga0_bit_mapping.size(); iChan++) { // FPGA0
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_TX_LVDS_P[" << iChan << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_TX_LVDS_N[" << iChan << "] ]" << std::endl;
  ...
  for (int iChan=0; iChan<tx_fmc_to_fpga1_bit_mapping.size(); iChan++) { // FPGA1
    ...
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_TX_LVDS_P[" << channel_id_total << "] ]" << std::endl;
    std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_TX_LVDS_N[" << channel_id_total << "] ]" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_TX_FWD_CLK_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_TX_FWD_CLK_N]" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_SOFT_RESET0_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_SOFT_RESET0_N]" << std::endl;
  //std::cout<<"NET FB2_SOFT_RESET0_P LOC="<<(pins_pos_la_fmcs[1])[soft_reset_fmc_to_fpga_bit_mapping[0]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  //std::cout<<"NET FB2_SOFT_RESET0_N LOC="<<(pins_neg_la_fmcs[1])[soft_reset_fmc_to_fpga_bit_mapping[0]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  ...
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_SOFT_RESET1_P]" << std::endl;
  std::cout << "set_property IOSTANDARD LVDS_25 [get_ports FB2_SOFT_RESET1_N]" << std::endl;
  //std::cout<<"NET FB2_SOFT_RESET1_P LOC="<<(pins_pos_la_fmcs[1])[soft_reset_fmc_to_fpga_bit_mapping[1]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
  //std::cout<<"NET FB2_SOFT_RESET1_N LOC="<<(pins_neg_la_fmcs[1])[soft_reset_fmc_to_fpga_bit_mapping[1]]<<" | IOSTANDARD=LVDS_25;" << std::endl;
